...
Even in state-of-the-art aberration-corrected STEM microscopes, we can never truly eliminate higher-order residual aberrations, and in-fact for certain imaging modalities we deliberately introduce aberrations to structure the incoming illumination.
As such, it is important to understand how to characterize these deviations and how they affect the incoming illumination.

## Converged Electron Probes

In what follows, we will assume off-axial aberrations have been corrected and investigate a converged probe illumination as a coherent wavefunction.
...
```
where $A(\bm{k})$ is the probe-forming aperture, $\chi(\bm{k})$ is the aplanatic aberration function, and $\mathcal{F}$ is the Fourier transform operator.

### Probe-Forming Aperture

As we've already seen, the STEM probe-forming aperture, located in the condenser system, limits the maximum wavevector of the incident electron wavefunction.
